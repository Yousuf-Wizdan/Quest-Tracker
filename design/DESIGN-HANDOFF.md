# e35bcb23-6d76-4715-90ce-3f07c86482d2 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `ascend-status-window.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `ascend-status-window.html`
...
- A single self-contained `ascend-status-window.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `ascend-status-window.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `ascend-status-window.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
